...
def save_pool():
    for xi in range(total_models):
        current_pool[xi].save_weights("training_model/model_new" + str(xi) + ".keras")
    print("Saved current trained_pool!")

# GA crossover
def ga_crossover(model_idx1, model_idx2):
    global current_pool
...
if load_saved_pool:
    for i in range(total_models):
        current_pool[i].load_weights("training_model/model_new"+str(i)+".keras")
    ...
    for select in range(int(total_models/2)):
        ...
        for idxx in range(total_models):
            if fitness[idxx] >= parent2:
                idx2 = idxx
                break
        new_weights1 = ga_crossover(idx1, idx2)
        updated_weights1 = ga_mutate(new_weights1[0])
        updated_weights2 = ga_mutate(new_weights1[1])
...
def checkCrash(players, upperPipes, lowerPipes):
    statuses = []
    for idx in range(total_models + 1):
